...
export const ChatWindow: React.FC<ChatWindowProps> = ({ 
  ...
  if (!chat) return null; // Simplified null check for brevity
  ...
  if (showPollCreator) {
    return (
      <div className="flex-1 flex items-center justify-center bg-gray-100/50 dark:bg-black/50 backdrop-blur z-50 p-4">
  ...
  if (showMediaGallery) {
      return (
          <div className="flex h-full bg-white dark:bg-dark-bg flex-col animate-fade-in">
  ...
  return (
    <div className="flex h-full overflow-hidden bg-gray-100 dark:bg-dark-bg transition-colors">
      <div className="flex-1 flex flex-col relative h-full">
        {/* Header */}
        <div className="absolute top-0 left-0 right-0 h-16 sm:h-20 flex flex-col z-30 transition-all">
            <div className="flex items-center justify-between px-4 sm:px-6 h-full bg-white/80 dark:bg-dark-panel/80 backdrop-blur-md border-b border-gray-200/50 dark:border-gray-700/50 shadow-sm">
                <div className={`flex items-center justify-between w-full transition-opacity duration-200 ${showSearch ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                        <div className="flex items-center cursor-pointer flex-1 min-w-0" onClick={() => setShowContactInfo(true)}>
                            ...
                            <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 font-medium truncate">
                                {partnerTyping ? <span className="text-nexus-500 animate-pulse font-semibold">Typing...</span> : partner.status === 'online' ? 'Online' : 'Offline'}
                            </p>
                            </div>
                        </div>
            ...
            {pinnedMessage && (
                <div onClick={() => document.getElementById(`msg-${pinnedMessage.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })} className="bg-gray-100/90 dark:bg-gray-800/90 backdrop-blur text-xs flex items-center justify-between px-4 py-2 cursor-pointer border-b border-gray-200 dark:border-gray-700 shadow-sm animate-fade-in">
        ...
        {/* Messages Area */}
        <div className="flex-1 overflow-y-auto p-4 pt-28 pb-4 z-0 bg-cover bg-center relative scrollbar-hide" style={{ backgroundImage: `url(${wallpaper})` }}>
                ...
                return (
                  ...
                  >
                    ...
                    >
                                ...
                                {msg.type === MessageType.POLL && msg.pollOptions && (
                                   <div className="min-w-[200px] space-y-3">
                                       ...
                                       {msg.pollOptions.map(opt => {
                                           ...
                                           return (
                                               <div key={opt.id} onClick={() => onVotePoll(msg.id, opt.id)} className="cursor-pointer relative overflow-hidden rounded-lg bg-black/10 dark:bg-white/10 p-2">
                      ...
                      {/* QUICK ACTIONS TOOLBAR - NEW LAYOUT (ABOVE MESSAGE) */}
                      {!isDeleted && (
                          <div className={`
